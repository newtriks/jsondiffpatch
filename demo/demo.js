(function demo(){
    ...
    load.data = function(data) {
        ...
        dom.text(document.getElementById('json-panel-left').querySelector('h2'),
            (data.left && data.left.name) || 'left.json');
        dom.text(document.getElementById('json-panel-right').querySelector('h2'),
    ...
    load.key = function(key) {
        ...
        load.data({ error: 'unsupported source: ' + key });
    };
        ...
        switch(example) {
            ...
            case 'query':
                document.location = '?desc=encoded%20in%20url&left=' +
                    ...
                    encodeURIComponent(JSON.stringify({"don't":"abuse","with": ["large","urls"]})) +
                    '&right=' +
                    encodeURIComponent(JSON.stringify({"don't":"use","with": [">", 2,"KB urls"]}));
                break;
            case 'urls':
